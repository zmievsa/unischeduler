...
def parse_schedule(schedule: str) -> List[Union[str, List[str]]]:
    schedule = "\n".join([l.strip() for l in schedule.splitlines() if l.strip()])
    ...
    for class_summary, class_info in zip(class_summaries, classes):
        ...
        if "Dropped" in class_info or "Withdrawn" in class_info:
            continue
        ...
        for section in current_sections:
            ...
            sections.append(
                [class_summary, section_type, weekdays, start_time, end_time, location, professors, dtstart, dtend]
            )
    return sections
